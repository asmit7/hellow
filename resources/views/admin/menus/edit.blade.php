        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="p-2 m-2 bg-slate-100 rounded">
                <div class="mt-10 w-1/2 space-y-8 divide-gray-200">
                      <form method="POST" enctype="multipart/form-data" action="{{ route('admin.menues.update', $menus->id) }}">
                       @csrf  
                       @method('PUT')
                        <div class="sm:col-span-6">
                            <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                            <div class="mt-1">
                                <input type="text" id="title" name="title" value="{{ $menus->name }}" class="block w-full transition
                        ...
                        <div class="sm:col-span-6">
                          ...
                          <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="number" id="price" min="0.00" max="10000" name="price" value="{{ $menus->price }}" class="block w-full transition
                                 duration-150 ease-in-out appearance  bg-white border   border-gray-400 rounded" />
                            </div>
                            @error('price')
                            <div class="text-sm text-red mb-2">{{$message}}</div>
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <textarea id="description" name="description" rows="3" class="block w-full shadow-sm focus:ring-indigo-500 duration-150 transition ease-in-out appearance
                                      bg-white border border-gray-400 rounded">{{ $menus->description }}
                             </textarea>
                             @error('description')
                            <div class="text-sm text-red mb-2">{{$message}}</div>
                            @enderror
                        ...
                        <div class="mt-6 p-4"> <button type="submit" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Update</button>
                        </div>
                    </form>
                </div>
            </div>
